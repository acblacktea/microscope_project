...
MainWidget::MainWidget(QWidget* parent)
    ...
    , m_frame(0), m_count(0)
    , m_edf(nullptr), m_pDataedf(nullptr)
{
    ...
    connect(this, &MainWidget::imgECallback, this, [this](eImageproEdfEvent nEvent)
    {
        if (m_hcam)
        {
            if (eImageproEdf_ERROR == nEvent)
            {
                closeCamera();
                QMessageBox::warning(this, "Warning", "Edf generic error");
            }
            else if (eImageproEdf_NOMEM == nEvent)
            {
                closeCamera();
    ...
    connect(this, &MainWidget::imgCallback, this, [this]()
    {
        if (m_edf)
        {
            imagepro_edf_readdata(m_edf, m_pDataedf, TDIBWIDTHBYTES(m_imgWidth * 24));
            QImage image(m_pDataedf, m_imgWidth, m_imgHeight, QImage::Format_RGB888);
...
void MainWidget::openCamera(const wchar_t* id)
{
    ...
    if (m_hcam)
    {
        ...
        m_pDataedf = new uchar[TDIBWIDTHBYTES(m_imgWidth * 24) * m_imgHeight];
        if (SUCCEEDED(Uvcham_start(m_hcam, nullptr/* Pull Mode */, eventCallBack, this)))
        {
            ...
            m_edf = imagepro_edf_newV2(eImageproFormat_RGB24, eImageproEdfM_Pyr_Weighted, imageCallBack, imageECallBack, this);
...
void MainWidget::onBtnSnap()
{
    ...
    if (m_edf)
    {
        imagepro_edf_readdata(m_edf, &vec[0], TDIBWIDTHBYTES(m_imgWidth * 24));
...
void MainWidget::imageCallBack(void* ctx, int result, void* outData, int stride, int outW, int outH, int outType)
{
    MainWidget* pThis = reinterpret_cast<MainWidget*>(ctx);
    emit pThis->imgCallback();
}

void MainWidget::imageECallBack(void* ctx, eImageproEdfEvent evt)
{
    MainWidget* pThis = reinterpret_cast<MainWidget*>(ctx);
    emit pThis->imgECallback(evt);
}

void MainWidget::onImageEvent()
{
    if (SUCCEEDED(imagepro_edf_pullham(m_edf, m_hcam, 1, m_imgWidth, m_imgHeight, m_pData))) /* Pull Mode */
    {
		++m_frame;
